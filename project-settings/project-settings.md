...
2. Kliknij zakładkę **Ustawienia projektu**  <img src="../.gitbook/assets/icon_project-settings.JPG" alt="" data-size="line"> na lewym pasku bocznym
...
* **Kiedy wyłączyć**: Wyłącz tylko wtedy, gdy kombinacja aparatu/obiektywu zastosowała już korekcję winietowania lub jeśli chcesz ręcznie skorygować winietowanie w postprodukcji.

### Kalibracja odbicia / balans bieli

* **Typ**: Pole wyboru
* **Domyślnie**: Włączone (zaznaczone)
* **Opis**: Włącza automatyczną kalibrację odbicia światła przy użyciu wykrytych celów kalibracyjnych w obrazach. Normalizuje to wartości odbicia światła w całym zbiorze danych i zapewnia spójne pomiary niezależnie od warunków oświetleniowych.
...
### Minimalny odstęp czasu między ponownymi kalibracjami

* **Typ**: Liczba
* **Zakres**: od 0 do 3600 sekund
* **Domyślnie**: 0 sekund
* **Opis**: Ustawia minimalny odstęp czasu (w sekundach) między użyciem celów kalibracyjnych. Po ustawieniu wartości 0 program Chloros będzie używał wszystkich wykrytych celów kalibracyjnych. Po ustawieniu wyższej wartości program Chloros będzie używał tylko celów kalibracyjnych, które są oddzielone co najmniej taką liczbą sekund, co skraca czas przetwarzania zestawów danych z częstym przechwytywaniem celów kalibracyjnych.
* **Kiedy dostosować**:
...
* **Kiedy dostosować**: Ustaw tę opcję zgodnie z przesunięciem strefy czasowej lokalnej, jeśli dane PPK wykorzystują czas lokalny zamiast UTC. Na przykład:
...
* **Opis**: Umożliwia stosowanie korekt kinematycznych przetworzonych po zakończeniu procesu (PPK) z rejestratorów MAPIR DAQ zawierających GPS (GNSS). Po włączeniu Chloros będzie używać wszystkich plików dziennika .daq zawierających dane pinów ekspozycji w katalogu projektu i stosować precyzyjne korekty geolokalizacji do obrazów.
* **Wymagania**: plik dziennika .daq z wpisami pinów ekspozycji musi znajdować się w katalogu projektu
* **Kiedy włączyć**: Zaleca się, aby zawsze włączać korektę PPK, jeśli w pliku dziennika .daq znajdują się wpisy dotyczące ekspozycji.

### Pin ekspozycji 1

* **Typ**: Wybór z listy rozwijanej
* **Widoczność**: Widoczny tylko wtedy, gdy włączona jest opcja „Zastosuj korekty PPK” ORAZ dostępne są dane ekspozycji dla pinu 1
* **Opcje**:
...
* **Typ**: Specjalny panel konfiguracji indeksu
* **Opis**: Otwiera interaktywny panel, w którym można wybrać i skonfigurować wielospektralne indeksy wegetacji (NDVI, NDRE, EVI itp.) do obliczenia podczas przetwarzania obrazu. Można dodać wiele indeksów, każdy z własnymi ustawieniami wizualizacji.
* **Dostępne indeksy**: System zawiera ponad 30 predefiniowanych indeksów wielospektralnych, w tym:
  * NDVI (znormalizowany indeks różnicy wegetacji)
  * NDRE (znormalizowana różnica RedEdge)
  * EVI (ulepszony indeks wegetacji)
  * GNDVI, SAVI, OSAVI, MSAVI2
  * I wiele innych (pełna lista znajduje się w sekcji [Wzory indeksów wielospektralnych](multispectral-index-formulas.md) w celu uzyskania pełnej listy)
* **Funkcje**:
  * Wybór spośród predefiniowanych formuł indeksów
...
* **Jak utworzyć**:
  1. W panelu konfiguracji indeksu znajdź opcję formuł niestandardowych
...
* **Zalecane formaty**:
  ...
  * **JPG (8-bit)**: Najmniejsze rozmiary plików, najlepsze tylko do podglądu i wyświetlania w Internecie. Wykorzystuje kompresję stratną, która nie nadaje się do analiz naukowych.
...
* **Kiedy zmienić**:
  * Ustaw dysk sieciowy do współpracy zespołowej.
  * Zmień na dysk z większą przestrzenią dyskową dla dużych zestawów danych.
...
3. **Przetestuj przed pełnym przetworzeniem**: Eksperymentując z nowymi ustawieniami, przetestuj je na niewielkiej podgrupie obrazów przed przetworzeniem całego zestawu danych.
